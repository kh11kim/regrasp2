...
from regrasp.vgn.grasp import *
from regrasp.utils.transform import Transform, Rotation
from regrasp.vgn.networks import load_network
import open3d as o3d

class VGN(object):
    def __init__(self, model_path: str, rviz=False):
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.net = load_network(Path(model_path), self.device)
        self.rviz = rviz
...
def process(
    ...
    outside_voxels = tsdf_vol > 0.4
    inside_voxels = np.logical_and(1e-3 < tsdf_vol, tsdf_vol < 0.4)
...
def select(qual_vol, rot_vol, width_vol, threshold=0.90, max_filter_size=3):
